...
from ..viz import (
    ...
    plot_distillation_impact,
    render_dispatch_diagram_svg,
)
...
# The independently-trained draft model's acceptance rate before the switch
# to distillation, as actually observed across several runs at the time (see
# the README's distillation story). That training path no longer exists in
# this codebase (distillation replaced it outright), so this range is quoted
# as a documented historical measurement, not regenerated by this script.
PRE_DISTILLATION_ACCEPTANCE_RANGE = (3.0, 7.0)
...
def main() -> None:
    ...
    low, high = PRE_DISTILLATION_ACCEPTANCE_RANGE
    plot_distillation_impact(
        low, high, stats.accepted / stats.proposed * 100,
        ASSETS_DIR / "distillation_impact.png",
    )
    render_dispatch_diagram_svg(ASSETS_DIR / "dispatch_diagram.svg")
    print(f"wrote {ASSETS_DIR / 'decoding_comparison.png'}")
    print(f"wrote {ASSETS_DIR / 'acceptance_rate.png'}")
    print(f"wrote {ASSETS_DIR / 'distillation_impact.png'}")
    print(f"wrote {ASSETS_DIR / 'dispatch_diagram.svg'}")
